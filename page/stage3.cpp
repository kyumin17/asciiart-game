...
void printStage3Page() {
    ...
    Enemy enemy(100, 1, 70, 0, enemyImage.width, enemyImage.height, 20, enemyImage.sunList[0]);
    Input input;
    input.isQuit = false;
    ...
    while (!input.isQuit && player.hp != 0 && enemy.hp != 0) {
        ...
        display.printBackground(player.hp, enemy.hp, "SUN");
